...
export const CartProvider = ({ children }) => {
  ...
  const fetchCart = async () => {
    ...
    if (!token || !apiBaseUrl || user?.role !== 'buyer') return;

    try {
      const response = await fetch(`${apiBaseUrl}/buyer/cart`, {
        method: 'GET',
        headers: {
  ...
  const addToCart = async (product, quantity = 1) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/buyer/cart/add`, {
        method: 'POST',
        headers: {
  ...
  const removeFromCart = async (productId) => {
    try {
      ...
      if (item) {
        ...
        const apiBaseUrl = getBackendUrl();
        if (token && apiBaseUrl) {
          const response = await fetch(`${apiBaseUrl}/buyer/cart/product/${productId}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`
            }
  ...
  const clearCart = async () => {
    try {
      ...
      if (token && apiBaseUrl) {
        const response = await fetch(`${apiBaseUrl}/buyer/cart/clear`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
