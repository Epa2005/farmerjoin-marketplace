      ...
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-4">{t('cartEmpty', 'Your cart is empty')}</h2>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {items.reduce((total, item) => total + item.quantity, 0)} {t('itemsInCart', 'items in your cart')}
              </p>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-8">
              <div className="p-6">
                <h2 className="text-lg font-bold text-gray-900 mb-4">{t('orderSummary', 'Order Summary')}</h2>

                <div className="space-y-3 mb-6">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">{t('subtotal', 'Subtotal')} ({items.reduce((total, item) => total + item.quantity, 0)} {t('items', 'items')})</span>
                    <span className="font-medium text-gray-900">RWF {subtotal.toFixed(0)}</span>
                  </div>

                  {discount > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-green-600">{t('discount', 'Discount')} ({(discount * 100).toFixed(0)}%)</span>
                      <span className="font-medium text-green-600">-RWF {discountAmount.toFixed(0)}</span>
                    </div>
                  )}

                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">{t('deliveryFee', 'Delivery Fee')}</span>
                    <span className="font-medium text-gray-900">RWF {deliveryFee.toFixed(0)}</span>
                  </div>

                  <div className="border-t border-gray-200 pt-3">
                    <div className="flex justify-between">
                      <span className="text-base font-bold text-gray-900">{t('total', 'Total')}</span>
                ...
                <div className="space-y-3">
                  <Link
                    to="/checkout"
                    className="block w-full text-center px-6 py-3 bg-gradient-to-r from-emerald-500 to-cyan-600 text-white font-semibold rounded-lg hover:from-emerald-600 hover:to-cyan-700 transition-all duration-300 shadow-lg hover:shadow-xl"
                  >
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  {discount > 0 && (
                    <p className="mt-2 text-sm text-green-600">{t('discountApplied', 'Discount applied!')}</p>
                  )}
                </div>
              </div>
            </div>
