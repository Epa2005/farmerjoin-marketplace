...
function Products() {
    ...
    const [productsPerPage] = useState(12); // Number of products to show at once
    ...
    const getPopularityBadge = (product) => {
        ...
        if (unitsSold >= 100 || timesOrdered >= 50) {
            return { label: 'Best Seller', className: 'bg-amber-500 text-white' };
        }
        if (unitsSold >= 40 || timesOrdered >= 20) {
            return { label: 'Trending', className: 'bg-violet-600 text-white' };
        }
        if (unitsSold >= 10 || timesOrdered >= 5) {
            return { label: 'Popular', className: 'bg-blue-600 text-white' };
        }
        return null;
    };
    ...
    // Random product display logic
    useEffect(() => {
        if (products.length > 0) {
            // Get previously shown product IDs from localStorage
            const shownProductIds = JSON.parse(localStorage.getItem('shownProductIds') || '[]');
            
            // Filter out already shown products
            const unseenProducts = products.filter(p => !shownProductIds.includes(p.product_id));
            
            // If all products have been shown, reset and start over
            let productsToDisplay;
            if (unseenProducts.length === 0) {
                localStorage.setItem('shownProductIds', '[]');
                productsToDisplay = shuffleArray([...products]).slice(0, productsPerPage);
            } else {
                // Randomly select from unseen products
                productsToDisplay = shuffleArray(unseenProducts).slice(0, productsPerPage);
            }
            
            // Update shown product IDs
            const newShownIds = [...shownProductIds, ...productsToDisplay.map(p => p.product_id)];
            localStorage.setItem('shownProductIds', JSON.stringify(newShownIds));
            
            setDisplayedProducts(productsToDisplay);
            setHasMore(unseenProducts.length > productsPerPage);
        }
    }, [products]);
    ...
    const loadMoreProducts = () => {
        ...
        if (unseenProducts.length > 0) {
            const newProducts = shuffleArray(unseenProducts).slice(0, productsPerPage);
            const newShownIds = [...shownProductIds, ...newProducts.map(p => p.product_id)];
            localStorage.setItem('shownProductIds', JSON.stringify(newShownIds));
            
            setDisplayedProducts([...displayedProducts, ...newProducts]);
            setHasMore(unseenProducts.length > productsPerPage);
        } else {
            // Reset and show random products from all
            localStorage.setItem('shownProductIds', '[]');
            const reshuffled = shuffleArray([...products]).slice(0, productsPerPage);
            const newShownIds = reshuffled.map(p => p.product_id);
            localStorage.setItem('shownProductIds', JSON.stringify(newShownIds));
            
            setDisplayedProducts(reshuffled);
            setHasMore(products.length > productsPerPage);
        }
    };

    // Refresh to show different random products
                ...
                <div className="max-w-7xl mx-auto px-4 py-4">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div className="flex items-center text-sm text-gray-600">
                            <Link to="/" className="hover:text-emerald-600">{t('home', 'Home')}</Link>
                            <span className="mx-2">/</span>
                            <span className="text-gray-900 font-medium">{t('products', 'Products')}</span>
                        </div>
                        ...
                        <div className="text-sm text-gray-600">
                            {displayedProducts.length} {t('product', 'product')}{displayedProducts.length !== 1 ? 's' : ''} {t('showing', 'showing')}
                            {products.length > displayedProducts.length && ` ${t('of', 'of')} ${products.length}`}
                            {(buyerDistrict || buyerSector || location.state?.district || location.state?.sector) && farmers.length > 0 && (
                                <span className="text-emerald-600"> {t('from', 'from')} {farmers.length} {t('farmer', 'farmer')}{farmers.length !== 1 ? 's' : ''} {t('nearYou', 'near you')}</span>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="lg:w-64 flex-shrink-0">
                        <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                            <h3 className="font-bold text-gray-900 mb-4">{t('filters', 'Filters')}</h3>

                            {/* Location Filter */}
                            <div className="mb-6">
                                <h4 className="font-medium text-gray-900 mb-3">{t('yourLocation', 'Your Location')}</h4>
                                <div className="space-y-3">
                                    {/* Province Input */}
                                    <div>
                                        <label className="text-xs text-gray-500 mb-1 block">{t('province', 'Province')}</label>
                                        <input
                                            type="text"
                                    ...
                                    {/* District Dropdown */}
                                    <div>
                                        <label className="text-xs text-gray-500 mb-1 block">{t('district', 'District')}</label>
                                        <select
                                            value={buyerDistrict}
                                            onChange={(e) => {
                                    ...
                                    {/* Sector Dropdown */}
                                    <div>
                                        <label className="text-xs text-gray-500 mb-1 block">{t('sector', 'Sector')}</label>
                                        <select
                                            value={buyerSector}
                                        ...
                                        >
                                            <option value="">
                                                {!buyerDistrict ? t('selectDistrictFirst', 'Select District First') : t('selectSector', 'Select Sector')}
                                            </option>
                                    ...
                                    {/* Cell Input */}
                                    <div>
                                        <label className="text-xs text-gray-500 mb-1 block">{t('cellOptional', 'Cell (Optional)')}</label>
                                        <input
                                            type="text"
                                            placeholder={t('enterCell', 'Enter cell')}
                                            value={buyerCell}
                                    ...
                                    >
                                        {t('findNearbyProducts', 'Find Nearby Products')}
                                    </button>
                                </div>
                            </div>
                            ...
                            {/* Categories */}
                            <div className="mb-6">
                                <h4 className="font-medium text-gray-900 mb-3">{t('categories', 'Categories')}</h4>
                            ...
                            {/* Price Range */}
                            <div className="mb-6">
                                <h4 className="font-medium text-gray-900 mb-3">{t('priceRange', 'Price Range')}</h4>
                            ...
                            {/* Availability */}
                            <div className="mb-6">
                                <h4 className="font-medium text-gray-900 mb-3">{t('availability', 'Availability')}</h4>
                                <label className="flex items-center cursor-pointer">
                                    ...
                                    />
                                    <span className="ml-2 text-sm text-gray-700">{t('inStockOnly', 'In Stock Only')}</span>
                                </label>
                            </div>
                            ...
                            {selectedFarmer && (
                                <div className="mb-6 p-3 bg-emerald-50 rounded-lg">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-sm font-medium text-gray-900">{t('selectedFarmer', 'Selected Farmer')}</span>
                                        <button onClick={clearFarmerSelection} className="text-xs text-red-600 hover:text-red-700">{t('clear', 'Clear')}</button>
                                    </div>
                                    <p className="text-sm text-gray-700">{selectedFarmer.full_name}</p>
                                    <p className="text-xs text-gray-500">{selectedFarmer.location || t('locationNotSpecified', 'Location not specified')}</p>
                                </div>
                            )}
                            ...
                            <div className="mb-6 bg-blue-50 border border-blue-200 rounded-xl p-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-gray-900">{t('setYourLocation', 'Set Your Location')}</h3>
                                            <p className="text-sm text-gray-600 mt-1">
                                                {t('findProductsFromFarmersNearYou', 'Find products from farmers near you for fresher produce and faster delivery')}
                                            </p>
                                        </div>
                                    </div>
                                    <button
                        ...
                        {farmers.length > 0 && !selectedFarmer && (
                            <div className="mb-6">
                                <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                                    ...
                                    {t('farmersNearYou', 'Farmers Near You')}
                                </h3>
                            ...
                            <div className="mb-6 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-gray-900">
                                                {t('productsFrom', 'Products from')} {selectedFarmer.full_name || selectedFarmer.farmer_name}
                                            </h3>
                                            <p className="text-sm text-gray-600">
                                                {selectedFarmer.location || selectedFarmer.farmer_district || selectedFarmer.district || t('locationNotSpecified', 'Location not specified')}
                                            </p>
                                        </div>
                                    </div>
                                    <button
                                            ...
                                            <div className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
                                                ...
                                                <div className="relative aspect-square bg-gray-100">
                                                    {product.image || product.image_url ? (
                                                        <img
                                                            src={buildImageUrl(product.image || product.image_url)}
                                                            alt={product.product_name}
                                                            className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 ${product.quantity <= 0 ? 'opacity-60 grayscale' : ''}`}
                                                        />
                                                    ) : (
                                                    ...
                                                    {product.quantity <= 0 && (
                                                        <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                                                            <span className="bg-red-600 text-white text-xs md:text-sm px-3 py-1.5 rounded-full font-semibold tracking-wide">
                                                                Out of Stock
                                                            </span>
                                                        </div>
                                                    )}
                                                    ...
                                                    <div className="absolute top-3 left-3 flex flex-col gap-2">
                                                        {getPopularityBadge(product) && product.quantity > 0 && (
                                                            <div className={`${getPopularityBadge(product).className} text-xs px-2 py-1 rounded-full font-medium`}>
                                                                {getPopularityBadge(product).label}
                                                            </div>
                                                        )}
                                ...
                                <div className="flex justify-center gap-4 mt-8">
                                    {hasMore && (
                                        <button
                                            ...
                                            className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition-colors shadow-lg hover:shadow-xl"
                                        >
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-3 bg-white border border-gray-300 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition-colors shadow-sm hover:shadow-md flex items-center gap-2"
                                    >
                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
