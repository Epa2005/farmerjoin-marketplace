...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.message === "Email already exists" || err.response?.data?.message === "Email already registered") {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <h2 className="text-4xl font-bold text-gray-800 mb-4">{t('createAccount', 'Create Account')}</h2>
          <p className="text-lg text-gray-600">{t('joinMarketplace', 'Join our agricultural marketplace')}</p>
        </div>
