                ...
                <div className="relative z-10 text-center text-white px-4 max-w-7xl mx-auto">
                    ...
                    <div className="absolute top-0 right-0">
                        <select
                            ...
                            className="px-3 py-2 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                        >
                            ...
                            {products.slice(0, 8).map((product) => (
                                <Link key={product.product_id} to={`/products/${product.product_id}`} className="group">
                                    <div className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
                                        {/* Product Image */}
                                        <div className="relative aspect-square bg-gray-100 overflow-hidden rounded-t-lg group-hover:shadow-xl transition-shadow duration-300">
                                            {product.image ? (
                                                <img
                                                    src={buildImageUrl(product.image)}
                                                    alt={product.product_name}
                                                    loading="lazy"
                                                    className={`w-full h-full object-cover transition-transform duration-500 transform-gpu group-hover:scale-105 group-hover:brightness-105 ${product.quantity <= 0 ? 'opacity-60 grayscale' : ''}`}
                                                />
                                            ) : (
                                            ...
                                            {getLikesCount(product) > 0 && product.quantity > 0 && (
                                                <div className="absolute top-2 right-2 bg-white/80 dark:bg-gray-800/70 text-gray-900 dark:text-white flex items-center gap-1 text-xs px-2 py-1 rounded-full backdrop-blur-sm shadow-sm">
                                                    ...
                                                    <span className="font-medium">{getLikesCount(product)}</span>
                                                </div>
                                            )}
                                            ...
                                            {getPopularityBadge(product) && product.quantity > 0 && (
                                                <span className={`${getPopularityBadge(product).className} absolute top-2 left-2 text-white text-xs px-2 py-1 rounded-full font-medium`}>
                                        ...
                                        <div className="p-4">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <span className="font-bold text-emerald-600">{t('currency', 'RWF')} {product.price}</span>
                                                <button
                                                    onClick={async (e) => {
                                                        e.preventDefault();
                                                        const token = localStorage.getItem('token');
                                            ...
                                            <div className="mt-3 md:hidden">
                                                <button
                                                    ...
                                                    className="w-full py-2 text-sm font-medium rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 transition-colors"
                                                >
                        ...
                        {products.slice(8, 12).map((product) => (
                            <Link key={product.product_id} to={`/products/${product.product_id}`} className="group">
                                <div className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
                                    ...
                                    {/* Mobile full-width add button for New Arrivals */}
