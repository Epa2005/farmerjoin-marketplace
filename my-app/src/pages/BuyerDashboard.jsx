...
function BuyerDashboard() {
  ...
  const fetchBuyerData = async (token) => {
    try {
      ...
      // Calculate stats from orders if stats endpoint doesn't provide data
      if (!statsRes.data || Object.keys(statsRes.data).length === 0) {
        const calculatedStats = {
          ...
          deliveredOrders: orders.filter(order => order.status === 'delivered' || order.status === 'completed').length
        };
      ...
      // Fallback: Try to fetch orders and calculate stats from them
      try {
        const ordersRes = await API.get("/orders/my-orders", {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        console.error("Error fetching orders as fallback:", ordersErr);
        setStats({
          totalOrders: 0,
          totalSpent: 0,
          pendingOrders: 0,
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{t('welcomeBack', 'Welcome back')}, {user?.full_name?.split(' ')[0] || t('buyer', 'Buyer')}!</h1>
              <p className="text-gray-600">{t('manageOrdersAccount', 'Manage your orders and account')}</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('totalOrders', 'Total Orders')}</p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{t('currency', 'RWF')} {Number(stats.totalSpent || 0).toLocaleString()}</p>
              </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('pendingOrders', 'Pending Orders')}</p>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('deliveredOrders', 'Delivered Orders')}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h2 className="text-lg font-bold text-gray-900 mb-4">{t('quickActions', 'Quick Actions')}</h2>
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-gray-900">{t('browseProducts', 'Browse Products')}</p>
                    <p className="text-xs text-gray-500">{t('shopNow', 'Shop now')}</p>
                  </div>
                </Link>
                <Link
                  to="/orders"
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-gray-900">{t('myOrders', 'My Orders')}</p>
                    <p className="text-xs text-gray-500">{t('trackOrders', 'Track orders')}</p>
                  </div>
                </Link>
                <Link
                  to="/edit-profile"
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-gray-900">{t('editProfile', 'Edit Profile')}</p>
                    <p className="text-xs text-gray-500">{t('updateInfo', 'Update info')}</p>
                  </div>
                </Link>
                <Link
                  to="/cart"
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-gray-900">{t('myCart', 'My Cart')}</p>
                    <p className="text-xs text-gray-500">{t('viewItems', 'View items')}</p>
                  </div>
                </Link>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-lg font-bold text-gray-900 mb-2">{t('myOrders', 'My Orders')}</h2>
                  <p className="text-sm text-gray-600 mb-4">Manage all your orders in one place. Track status, review items, and delete eligible orders.</p>
                  <div className="flex flex-wrap gap-2 text-xs">
                    <span className="px-3 py-1 rounded-full bg-emerald-100 text-emerald-700">{t('totalOrders', 'Total Orders')}: {stats.totalOrders}</span>
                    <span className="px-3 py-1 rounded-full bg-yellow-100 text-yellow-700">{t('pendingOrders', 'Pending Orders')}: {stats.pendingOrders}</span>
                    <span className="px-3 py-1 rounded-full bg-green-100 text-green-700">{t('deliveredOrders', 'Delivered Orders')}: {stats.deliveredOrders}</span>
                  </div>
                </div>
                <Link
                  to="/orders"
                  className="shrink-0 inline-flex items-center px-5 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg font-semibold hover:from-emerald-600 hover:to-teal-600 transition-all"
                >
                  Open Orders
                </Link>
              </div>
          ...
          <div className="space-y-8">
            {/* Delivery Location */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h2 className="text-lg font-bold text-gray-900 mb-4">{t('deliveryLocation', 'Delivery Location')}</h2>
              <form onSubmit={handleLocationSubmit} className="space-y-4">
                
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('district', 'District')}</label>
                  <select
                    value={district}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('sector', 'Sector')}</label>
                  <select
                    value={sector}
                  ...
                  >
                    <option value="">
                      {!district ? t('selectDistrictFirst', 'Select District First') : t('selectSector', 'Select Sector')}
                    </option>
              ...
              {savedLocation && (
                <div className="mt-4 p-4 bg-emerald-50 rounded-lg">
                  <p className="text-sm text-emerald-800">
                    <span className="font-semibold">{t('current', 'Current')}:</span>  {savedDistrict}, {savedSector}
                  </p>
                </div>
              )}
            </div>
            ...
            {/* Account Info */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h2 className="text-lg font-bold text-gray-900 mb-4">{t('accountInfo', 'Account Info')}</h2>
