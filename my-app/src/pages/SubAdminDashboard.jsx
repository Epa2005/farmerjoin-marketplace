...
function SubAdminDashboard() {
  ...
  const [locations, setLocations] = useState({ provinces: [], districts: [] });
  ...
  const fetchLocations = async () => {
    try {
      const res = await API.get('/api/ai/locations');
      if (res?.data) {
        setLocations({ provinces: res.data.provinces || [], districts: res.data.districts || [] });
      }
    } catch (e) {
      console.warn('Failed to load locations:', e?.message || e);
    }
  };

  // update district options whenever province changes
  useEffect(() => {
    if (!farmerData.province) {
      setDistrictOptions([]);
      setSectorOptions([]);
      return;
    }
    const districts = (locations.districts || []).filter(d => d.province === farmerData.province).map(d => d.district);
    setDistrictOptions(districts);
    setSectorOptions([]);
    setFarmerData(prev => ({ ...prev, district: prev.district && districts.includes(prev.district) ? prev.district : '', sector: '' }));
  }, [farmerData.province, locations]);

  // fetch sectors for selected province+district
  useEffect(() => {
    const prov = farmerData.province;
    const dist = farmerData.district;
    if (!prov || !dist) {
      setSectorOptions([]);
      return;
    }
    API.get(`/users/locations/sectors?province=${encodeURIComponent(prov)}&district=${encodeURIComponent(dist)}`)
      .then(r => setSectorOptions(r.data?.sectors || []))
      .catch(err => {
        console.warn('Failed to fetch sectors:', err?.message || err);
        setSectorOptions([]);
      });
  ...
  const fetchManagedFarmers = async () => {
    try {
      ...
      if (Array.isArray(response.data?.users)) {
        ...
        setFarmers(ensureArray(response.data));
      }
    } catch (err) {
  ...
  const fetchStatistics = async () => {
    try {
      ...
      const users = ensureArray(response.data?.users ?? response.data);
      setStatistics({
        ...response.data,
        total_users: response.data?.totalUsers ?? users.length,
  ...
  const handleEditFarmer = async () => {
    try {
      ...
      await API.put(`/farmers/admin/${farmerId}`, farmerData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      {/* Modern Sidebar */}
      <div className={`fixed left-0 top-0 h-full bg-black/20 backdrop-blur-xl border-r border-white/10 transition-all duration-300 z-30 ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 ${sidebarOpen ? 'w-64' : 'w-20'}`}>
        <div className="p-6">
      ...
      {/* Main Content Area */}
      <div className={`transition-all duration-300 min-h-screen overflow-y-auto ${sidebarOpen ? 'lg:ml-64' : 'lg:ml-20'}`}>
        {/* Modern Header */}
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center">
              <div className="flex items-start gap-3">
                <button
                  ...
                  className="lg:hidden mt-1 inline-flex items-center justify-center rounded-xl bg-white/10 p-2 text-white"
                >
                  <span className="text-lg">menu</span>
                </button>
                <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">{t('title', 'Sub Admin Dashboard')}</h1>
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 sm:gap-4">
                ...
                <select
                  ...
                  className="bg-white border border-gray-300 text-gray-900 px-4 py-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 cursor-pointer w-full sm:w-auto"
                >
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white px-4 sm:px-6 py-3 rounded-xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center space-x-2"
                >
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white px-4 sm:px-6 py-3 rounded-xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center space-x-2"
                >
                    ...
                    {orders.map((order) => (
                      <div key={order.order_id} className="bg-white/5 rounded-xl p-4 hover:bg-white/10 transition-all duration-300">
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${order.status === 'completed' ? 'bg-green-500/20 text-green-400' :
