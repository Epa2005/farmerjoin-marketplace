...
function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      try {
        const onlineUsersResponse = await API.get("/admin/online-users", {
          headers: { Authorization: `Bearer ${token}` }
        });
        setOnlineUsers(onlineUsersResponse.data?.users || []);
      } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <header className="bg-black/20 backdrop-blur-lg border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex min-h-16 flex-col gap-4 py-4 lg:h-16 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-white">{t('farmerJoinAdmin')}</h1>
            ...
            <div className="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center lg:w-auto">
              <select
                ...
                className="px-3 py-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full sm:w-auto"
              >
              ...
              <button
                ...
                className="bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all backdrop-blur-sm border border-white/20 w-full sm:w-auto"
              >
              ...
              <button
                ...
                className="bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-lg w-full sm:w-auto"
              >
                {t('logout')}
              </button>
            </div>
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex gap-1 overflow-x-auto bg-white/10 backdrop-blur-md rounded-lg p-1 border border-white/20">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-xl border border-white/20 p-6">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
                <div>
                  <h3 className="text-xl font-semibold text-white">Users Online Now</h3>
                  <p className="text-sm text-gray-300">People active in the last 5 minutes</p>
                </div>
                <div className="inline-flex items-center px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-sm font-semibold">
                  {onlineUsers.length} online
                </div>
              </div>

              {onlineUsers.length === 0 ? (
                <p className="text-gray-300 text-sm">No users are currently online.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {onlineUsers.slice(0, 9).map((onlineUser) => (
                    <div
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="font-semibold text-white truncate">{onlineUser.full_name || 'Unknown User'}</p>
                          <p className="text-sm text-gray-300 truncate">{onlineUser.email}</p>
                        </div>
                        <span className="shrink-0 rounded-full bg-emerald-500/20 px-2 py-1 text-xs font-semibold uppercase text-emerald-300">
                          {onlineUser.role}
                        </span>
                      </div>
                      <div className="mt-3 flex items-center justify-between text-sm">
                        ...
                        <span className="text-white">
                          {onlineUser.last_seen ? new Date(onlineUser.last_seen).toLocaleTimeString() : 'Just now'}
                        </span>
                      </div>
                    </div>
                  ))}
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full">
                    ...
                    {orders.map((order, index) => (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                          FRW {order.total_amount || (order.quantity * order.price).toFixed(0)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
