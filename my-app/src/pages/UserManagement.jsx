...
function UserManagement() {
  ...
  useEffect(() => {
    if (!subAdminData.province) {
      setDistrictOptions([]);
      setSectorOptions([]);
      return;
    }
    const districts = (locations.districts || []).filter(d => d.province === subAdminData.province).map(d => d.district);
    setDistrictOptions(districts);
    setSectorOptions([]);
    setSubAdminData(prev => ({ ...prev, district: '', sector: '' }));
  }, [subAdminData.province, locations]);

  // fetch sectors for selected province+district
  useEffect(() => {
    const prov = subAdminData.province;
    const dist = subAdminData.district;
    if (!prov || !dist) {
      setSectorOptions([]);
      setSubAdminData(prev => ({ ...prev, sector: '' }));
      return;
    }
  ...
  }, [subAdminData.province, subAdminData.district]);
  ...
  const fetchUsers = async () => {
    try {
      ...
      setUsers(ensureArray(response.data?.users ?? response.data));
    } catch (err) {
  ...
  const handleBanUser = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await API.put(`/users/${selectedUser.user_id}/ban`,
        { reason: banReason },
        {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSuspendUser = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await API.put(`/users/${selectedUser.user_id}/suspend`,
  ...
  const handleCreateSubAdmin = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await API.post('/users/sub-admin/create', subAdminData, {
        headers: { Authorization: `Bearer ${token}` }
      });
