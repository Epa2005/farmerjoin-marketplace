...
WHERE table_name = 'users'
  AND table_schema = ${usePostgres ? `'public'` : 'DATABASE()'}
`;

db.query(usersExistsQuery, (usersErr, usersResult) => {
    if (usersErr) {
        console.error('Error checking users table existence:', usersErr);
        return;
    }

    const hasUsersTable = (usersResult?.[0]?.table_count || 0) > 0;
    const createEnhancedLearningTable = usePostgres
        ? (hasUsersTable
            ? `
                ...
                CREATE INDEX IF NOT EXISTS idx_user_ml_interactions_user_created
                ON user_ml_interactions (user_id, created_at);
                CREATE INDEX IF NOT EXISTS idx_user_ml_interactions_question_type
                ON user_ml_interactions (question_type);
            `
            : `
                CREATE TABLE IF NOT EXISTS user_ml_interactions (
                    id SERIAL PRIMARY KEY,
    ...
    db.query(createEnhancedLearningTable, (err) => {
        if (err) {
            ...
            console.log(
                hasUsersTable
                    ? 'Enhanced ML interactions table created/verified with users FK'
                    : 'Enhanced ML interactions table created/verified without users FK (users table missing)'
            );
        }
        ...
        const mlFkCheckQuery = `
            SELECT COUNT(*) AS constraint_count
            FROM ${usePostgres ? 'information_schema.table_constraints' : 'information_schema.table_constraints'}
            WHERE table_name = 'user_ml_interactions'
              AND constraint_type = 'FOREIGN KEY'
              AND table_schema = ${usePostgres ? `'public'` : 'DATABASE()'}
        `;
        db.query(mlFkCheckQuery, (fkCheckErr, fkCheckResult) => {
            if (fkCheckErr) {
                console.error('Could not verify user_ml_interactions FK:', fkCheckErr);
                return;
            }

            const hasMlFk = (fkCheckResult?.[0]?.constraint_count || 0) > 0;
            if (hasMlFk) {
                return;
            }

            const addMlFkQuery = `
                ALTER TABLE user_ml_interactions
                ADD CONSTRAINT fk_user_ml_interactions_user
                FOREIGN KEY (user_id) REFERENCES users(user_id)
            `;
            db.query(addMlFkQuery, (addFkErr) => {
                if (addFkErr) {
                    console.error('Failed to add user_ml_interactions FK:', addFkErr);
                } else {
                    console.log('Repaired missing user_ml_interactions FK constraint successfully');
                }
            });
        });
    });
