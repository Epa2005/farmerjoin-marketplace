...
const HIDE_CHECKOUT = (process.env.HIDE_CHECKOUT === 'true' || process.env.HIDE_CHECKOUT === '1');
...
router.post('/mobile-money/checkout', async (req, res) => {
    if (HIDE_CHECKOUT) {
        return res.status(403).json({ success: false, message: 'Payments are currently disabled' });
    }
...
router.post('/mobile-money/callback', async (req, res) => {
    ...
    if (HIDE_CHECKOUT) {
        // Still acknowledge callback but do not process if payments disabled
        console.log('Payments disabled - callback ignored');
        return res.status(200).json({ message: 'Payments disabled - callback ignored' });
    }
    ...
    try {
        ...
        if (isSuccessful) {
            ...
            if (orderResult) {
                // Clear purchased items from buyer cart only after confirmed payment
                const clearPurchasedCartQuery = `
                    DELETE c
                    FROM cart c
                    JOIN orders o ON o.buyer_id = c.buyer_id
                    JOIN order_items oi ON oi.order_id = o.order_id AND oi.product_id = c.product_id
                    WHERE o.payment_reference = ? OR o.payment_id = ?
                `;

                await new Promise((resolve, reject) => {
                    db.query(clearPurchasedCartQuery, [reference, paymentId], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
            ...
            // Restore stock because it was reduced at checkout initiation
            const failedOrderItemsQuery = `
                SELECT oi.product_id, oi.quantity
                FROM order_items oi
                JOIN orders o ON oi.order_id = o.order_id
                WHERE o.payment_reference = ? OR o.payment_id = ?
            ...
            const failedOrderItems = await new Promise((resolve, reject) => {
                db.query(failedOrderItemsQuery, [reference, paymentId], (err, results) => {
                    if (err) reject(err);
                    else resolve(results || []);
                });
            });
