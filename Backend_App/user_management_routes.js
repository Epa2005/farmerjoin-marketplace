...
// Remove suspended/banned farmer products from buyer carts immediately
const removeFarmerProductsFromCarts = (userId, callback) => {
    const query = `
        DELETE FROM cart c
        USING products p
        JOIN farmers f ON p.farmer_id = f.farmer_id
        WHERE c.product_id = p.product_id AND f.user_id = ?
    `;

    db.query(query, [userId], (err, result) => {
        if (err) {
            console.error('Failed to remove farmer products from carts:', err);
            return callback(err);
        }

        console.log(`Removed ${result.affectedRows || 0} cart rows for moderated farmer user_id=${userId}`);
...
router.put('/:userId/ban', isAdmin, (req, res) => {
    ...
    db.query("UPDATE users SET status = 'banned' WHERE user_id = ?", [userId], (err, result) => {
        ...
        removeFarmerProductsFromCarts(userId, () => {
            return res.json({ 
...
// GET sectors for a given province and district (used by frontend cascading selects)
router.get('/locations/sectors', (req, res) => {
    ...
    const query = `
        SELECT DISTINCT sector FROM users WHERE LOWER(province) = LOWER(?) AND LOWER(district) = LOWER(?)
        UNION
        SELECT DISTINCT sector FROM farmers WHERE LOWER(province) = LOWER(?) AND LOWER(district) = LOWER(?)
    `;

    db.query(query, [province, district, province, district], (err, results) => {
        if (err) {
            console.error('Failed to fetch sectors:', err);
            return res.status(500).json({ sectors: [], error: err.message });
        }
        const sectors = results.map(r => r.sector).filter(Boolean);
        return res.json({ sectors });
    });
});
