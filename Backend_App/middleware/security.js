...
const createRateLimiter = (windowMs, max, message) => {
  return rateLimit({
    ...
    skip: (req) => {
      const publicReadPrefixes = ['/products', '/users', '/uploads', '/stats'];
      const isPublicRead = req.method === 'GET' && publicReadPrefixes.some(prefix => req.path.startsWith(prefix));
...
const generalLimiter = createRateLimiter(
  ...
  parseInt(process.env.GENERAL_RATE_LIMIT_MAX || '500', 10), // allow more normal browsing traffic in production
  'Too many requests from this IP, please try again later.'
);

const publicReadLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: parseInt(process.env.PUBLIC_READ_RATE_LIMIT_MAX || '1000', 10),
  message: { message: 'Too many public catalog requests, please try again later.' },
  standardHeaders: true,
  legacyHeaders: false,
  skip: (req) => {
    return process.env.NODE_ENV === 'development' || process.env.NODE_ENV === undefined;
