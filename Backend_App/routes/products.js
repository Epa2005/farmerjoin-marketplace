...
// Get products
router.get("/", (req, res) => {
  // Support simple pagination via ?limit= and ?offset=
  const limit = parseInt(req.query.limit, 10) || 0; // 0 means no limit
  const offset = parseInt(req.query.offset, 10) || 0;

  let sql = `SELECT products.product_id, products.farmer_id, products.product_name, products.category, products.price, products.quantity, products.image, products.created_at, users.full_name as farmer_name, users.photo as farmer_photo
     FROM products
     JOIN farmers ON products.farmer_id=farmers.farmer_id
     JOIN users ON farmers.user_id=users.user_id
     WHERE users.role = 'farmer'
       AND LOWER(COALESCE(users.status, 'inactive')) = 'active'`;
